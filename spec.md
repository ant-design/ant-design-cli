...
- `bundle-size.json` pre-computed during antd release CI (min+gzip per component)
...
antd diff 4.24.0 5.0.0 Select      # Select-specific changes
antd diff 5.0.0 6.0.0 --format json
```

Output includes: added props, removed props, changed types, renamed props. Cross-major-version diffing (e.g. v4 vs v5) is supported because `components.json` uses a consistent schema across versions.
...
Analyze bundle size impact of importing a component. Data is pre-computed during antd release CI and stored in `bundle-size.json`.
...
```json
{
  ...
  "size": {"raw": "128.5 kB", "gzip": "38.2 kB"},
  "dependencies": ["dayjs", "rc-picker", "rc-util"],
  "subComponents": [
    {"name": "DatePicker.RangePicker", "additionalSize": {"raw": "12.3 kB", "gzip": "3.8 kB"}}
  ]
}
```
...
│              │     │              │     │                  │
│  13 commands │     │  Version     │     │  Local cache     │
...
antd source code
  → extract-metadata.ts
    ...
    → build-time bundle analysis → per-component size data
