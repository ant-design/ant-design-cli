...
An extraction script runs offline against antd source (checked out locally or via git tags) to produce the bundled JSON files:
...
antd source code
  ...
  → write to data/v{major}.json
```

For historical versions (v4, v5), a one-time backfill script checks out each major version tag and runs the same extraction. Data is committed to the repo and published with each CLI release.
